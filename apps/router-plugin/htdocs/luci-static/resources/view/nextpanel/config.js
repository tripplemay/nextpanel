  ...
  render: function (data) {
    ...
    o.cfgvalue = function () {
      return [
        ...
        '<li>进入 <b>服务 → HomeProxy → 代理设置</b></li>',
        '<li>确认「路由模式」已自动切换为<b>自定义</b>（由 NextPanel 写入）</li>',
        '<li>将「代理模式」设为 <b>redirect + tproxy</b>（推荐）或 <b>TUN</b></li>',
        '<li>保存并应用</li>',
        '<li>点击右上角<b>启用</b>开关</li>',
        '</ol>',
        '<p style="margin-top:8px;color:#8c8c8c">',
        '以上步骤只需配置一次。同步后 NextPanel 自动写入全部规则并重载 HomeProxy，无需手动维护。',
        '</p>',
