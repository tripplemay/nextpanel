...
### 功能：路由测试 — 去程/回程延迟 + 路由追踪（Agent v1.5.0）

#### 新增能力

**回程（节点 → 中国）** — 由 Agent 在节点服务器上执行：
- 并发测试 9 个中国 ISP 节点（联通/电信/移动 × 北京/上海/广州）
- 指标：ICMP ping 延迟（ms）、TCP 握手延迟（ms）、丢包数
- 路由追踪：优先使用 `nexttrace`（含 ASN/机构注释），降级至 `traceroute`
- 总超时 90 秒，结果随 IP 检测一并上报

**去程（中国 → 节点）** — 由 Panel 调用第三方 API：
- 支持 itdog.cn 和 chinaz.com 两个数据源
- Provider 健康管理：连续 3 次失败后进入 5 分钟冷却，自动切换备选源
- 与回程数据合并后存储

#### UI 变更

- `IpCheckCard` 改为双 Tab 布局：「流媒体 / AI」和「路由测试」
- 去程展示：3 ISP × 3 城市延迟矩阵（颜色编码：绿 <80ms，黄 <150ms，红 ≥150ms）
- 回程展示：可折叠列表，含逐跳路由表（#、IP、ASN、机构、延迟）

#### 数据库

- `ServerIpCheck` 表新增 `routeData JSONB` 列（非破坏性变更）
